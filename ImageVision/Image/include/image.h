...
public:
    ...
    */
	...
	bool Create (int nWidth, int nHeight, int nColorBit,BYTE *pData);
    /// Destroy image.
    void Destroy() ;
    //@}
...
//-----------------------------------------------------------------------------------
inline bool FCObjImage::Create (int nWidth, int nHeight, int nColorBit,BYTE *pData)
{
	Destroy() ;
	...
	memcpy(m_line_ptr[0],pData,nPitch);
	for (int y=1 ; y < Height() ; y++)
	{
		m_line_ptr[y] = m_line_ptr[y - 1] - nPitch ;
		memcpy(m_line_ptr[y],pData+nPitch*y,nPitch);
	}
...
//-------------------------------------------------------------------------------------
inline void FCObjImage::ConvertToGray ()
{
    ...
    if (!Create (imgOld.Width(), imgOld.Height(), 8))
        return ;
    ...
    for (int y=0 ; y < Height() ; y++)
    {
        for (int x=0 ; x < Width() ; x++)
        {
            ...
            {
                case 8 : // 8 -->8
                    FCColor::CopyPixelBPP (pNew, pOld, 8) ;
                    break ;
                case 24 :
                case 32 : // 24,32 --> 8
					BYTE bPixel=FCColor::GetGrayscale(pOld);
                    FCColor::CopyPixelBPP (pNew,&bPixel , 8) ;
                    break ;
            }
        }
    }
