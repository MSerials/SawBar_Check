...
public:
    ...
    */
	...
	static RECT RectSizeWidthConstantRatio(RECT rcScreen,CSize sizePicture,BOOL bCenter=FALSE)
	{
		//BOOL bCenter=FALSE;
		CRect rcView(rcScreen);
		CRect rect(rcView);
		//CSize sizePicture(sizePicture.cx,sizePicture.cy);
		double dWidth=rcView.Width();
		double dHeight=rcView.Height();
		...
		//will do,otherwise need to calculate the new rectangle
		if(dPictureAspectRatio>dAspectRatio)
		{
			int nNewHeight=(int)(dWidth/dPictureWidth*dPictureHeight);
			int nCenteringFactor=(rcView.Height()-nNewHeight)/2;
			if(bCenter==FALSE)
				nCenteringFactor=0;
		...
		else if(dPictureAspectRatio<dAspectRatio)
		{
			...
			int nCenteringFactor=(rcView.Width()-nNewWidth)/2;
			if(bCenter==FALSE)
				nCenteringFactor=0;
			rect.SetRect(nCenteringFactor,0,nNewWidth+nCenteringFactor,(int)(dHeight));
		}

		//CRect rc=m_img.CalcFitWindowSize(CSize(m_img.Width(),m_img.Height()),rcView);
		RECT rc(rect);
		return rc;
	}
    /// @name Coordinate.
