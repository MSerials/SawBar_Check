...
inline void CCanvas::DrawGrid(CDC *dc, HWND hwnd)
{
...
inline void CCanvas::DrawHitMark(CDC *dc, int object_index)
{	
...
inline bool CCanvas::IsTooSmall()
{
...
inline int CCanvas::GetResizingDirection()
{
...
inline void CCanvas::ResizeObject()
{
	if (mIndex > -1)
	{
...
inline void CCanvas::SwapLong(LONG &a, LONG &b)
{
	LONG buf;
	buf = a;
	a = b;
...
inline bool CCanvas::FindObject(CPoint pt)
{
	DeselectAllObjects();
...
inline CPoint CCanvas::GetLineIntersectionWithObjects(CPoint &line_segment_begin, CPoint &line_segment_end)
{
...
inline void CCanvas::DeselectAllObjects()
{
...
inline void CCanvas::MoveSelectedObject()
{
	// Update movement
	for (UINT count = 0; count < mObjectContainer.size(); count++)
	{
...
inline void CCanvas::DrawFromContainer(CDC *dc)
{
	...
	for (UINT count = 0; count < mObjectContainer.size(); count++)
	{
		...
		Rect rr(r.left,r.top,r.Width(),r.Height());
		if (obj->type == TOOL_RECTANGLE)
			graph.DrawRectangle(&pen,rr);//dc->Rectangle(&r);
		else if (obj->type == TOOL_CIRCLE)
			graph.DrawEllipse(&pen,rr);//dc->Ellipse(&r);
		else if (obj->type == TOOL_LINE){
			//dc->MoveTo(obj->point[0]);
			//dc->LineTo(obj->point[1]);
			graph.DrawLine(&pen,Point(obj->point[0].x,obj->point[0].y),Point(obj->point[1].x,obj->point[1].y));
		}
...
inline void CCanvas::OnDraw(CDC *dc)
{
...
inline void CCanvas::OnMouseMove(CPoint pt)
{
	//SnapToGrid(pt);
...
inline void CCanvas::OnLButtonDown(CPoint pt)
{
	//SnapToGrid(pt);
	mLClickPos = pt;
...
inline void CCanvas::OnLButtonUp(CPoint pt)
{
	SnapToGrid(pt);
	mCurrentPos = pt;
...
inline int CCanvas::Zoom(float zoom_factor)
{
...
inline int CCanvas::DeleteSelectedObject()
{
...
inline void CCanvas::SetSnapToGrid(bool setting, int gridsizeX, int gridsizeY)
{
...
inline void CCanvas::SnapToGrid(CPoint &pt)
{
	if (mIsSnapToGrid)
		pt = CPoint(pt.x / mGridsizeX * mGridsizeX, pt.y / mGridsizeY * mGridsizeY);
}

inline void CCanvas::SetTool(int tool)
{
	DeselectAllObjects();
	mToolType = tool;
}
inline void CCanvas::AddObject(int itype,int iFunction,int iTarget,bool _bActive,CPoint pt1,CPoint pt2,bool bSelected)
{
	OBJECT* obj = (OBJECT*)malloc(sizeof(OBJECT));
	obj->type = itype;
	obj->point[0] = pt1;
	obj->point[1] = pt2;
	obj->iFunctionType=iFunction;
	obj->target_code=iTarget;
	obj->selected = bSelected;
	obj->bActive=_bActive;
	mObjectContainer.push_back(obj);
}
inline void CCanvas::StoreObject()
{
...
inline bool CCanvas::LoadFile(char *fileName)
{
...
inline bool CCanvas::SaveFile(char *fileName)
{
	if (mObjectContainer.empty())
	...
	for (UINT count = 0; count < mObjectContainer.size(); count++)
	{
		OBJECT *obj = mObjectContainer.at(count);
		file << obj->type << "\n";
		file << obj->target_code << "\n";
		file << obj->iFunctionType << "\n";
		file << obj->point[0].x << "\n";
		file << obj->point[0].y << "\n";
		file << obj->point[1].x << "\n";
		file << obj->point[1].y << "\n";
...
inline void CCanvas::ClearAll()
{
	mObjectContainer.clear();
}

inline void CCanvas::SetMessage(TCHAR message[128])
{
	_tcscpy(mMessage, message);
}