...
void CCreditStatic::SetCredits(LPCTSTR credits,char delimiter)
{
	...
	if((str = _strdup((LPSTR)(LPCTSTR)credits)) == NULL)
		return;
	...
	while((ptr2 = strchr(ptr1,delimiter)) != NULL) {
		*ptr2 = '\0';
		m_ArrCredit.AddTail((LPCTSTR)(LPSTR)ptr1);
		ptr1 = ptr2+1;
	}
	m_ArrCredit.AddTail((LPCTSTR)(LPSTR)ptr1);
...
BOOL CRegistrationTest::CheckRegistrationKey(LPCTSTR name, LPCTSTR key, bool suppressUI )
{
	...
	/*
BOOL ret;
CStringA userName(name);
CHAR *input;
WORD sum1=0;
CStringA appendSumText, totalSumText;
...
{
   AfxMessageBox(L"This is a fully functional version of MOD-SIM but without a key; \
