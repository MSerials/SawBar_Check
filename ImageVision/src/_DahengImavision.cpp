...
try
{
	...
	while(bGetImageOK==FALSE)
	{
		...
		//发送软触发命令
		GXSendCommand(m_hDevices[nCameraID],GX_COMMAND_TRIGGER_SOFTWARE);//GX_COMMAND_TRIGGER_SOFTWARE
	
		//调用GXGetImage取一帧图像
		Sleep(m_nSnapTimeDelay);
		
		//GXGetImage(m_hDevices[nCameraID],  m_stFrameDatas+nCameraID, 100);
		int m_nEditTimeOut = 1000;
	...
	else   
	{
		...
		{
			memcpy(m_pstCam[nCameraID].pImageBuffer+i*m_pstCam[nCameraID].nImageWidth, (BYTE *)(m_stFrameDatas[nCameraID].pImgBuf)+(m_stFrameDatas[nCameraID].nHeight-i-1)*m_stFrameDatas[nCameraID].nWidth,(size_t)m_pstCam[nCameraID].nImageWidth);
		}
	}
...
GX_STATUS CDahengImavision::InitDevice(int nCameraID)
{
	...
	//设置采集模式连续采集
	emStatus = GXSetEnum(m_hDevices[nCameraID],GX_ENUM_ACQUISITION_MODE, GX_ACQ_MODE_SINGLE_FRAME);
	VERIFY_STATUS_RET(emStatus);
	...
	//设置采集模式连续采集
	emStatus = GXSetEnum(m_hDevices[nCameraID],GX_ENUM_ACQUISITION_MODE, GX_ACQ_MODE_CONTINUOUS);//GX_ACQ_MODE_CONTINUOUS);
	VERIFY_STATUS_RET(emStatus);
